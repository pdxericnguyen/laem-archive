...
import {
  buildDevDiagnosticsSummary,
  listRecentStripePayments,
  type DevDiagnosticsProductState,
  type DevDiagnosticsStripePayment
} from "@/lib/dev-diagnostics";
import type { Product } from "@/lib/store";

export const metadata = { title: "Developer Diagnostics | Admin" };
export const dynamic = "force-dynamic";

type AdminDevDiagnosticsPageProps = {
...
async function loadStripeRows(sinceUnix: number) {
  ...
  if (!secretKey) {
    return {
      rows: [] as DevDiagnosticsStripePayment[],
...
export default async function AdminDevDiagnosticsPage({ searchParams }: AdminDevDiagnosticsPageProps) {
  ...
  if (!hasKvEnv()) {
    return (
      <main className="mx-auto max-w-6xl px-4 py-10 space-y-4">
        ...
        <h1 className="text-lg font-semibold tracking-tight">Developer Diagnostics</h1>
        <AdminCommandPalette />
  ...
  const productStates: DevDiagnosticsProductState[] = products.map((product) => ({
    slug: product.slug,
    title: product.title,
    ...
    <main className="mx-auto max-w-6xl px-4 py-10 space-y-8">
      ...
      <header className="space-y-2">
        ...
        <p className="text-sm text-neutral-600">
          Raw system checks for debugging rare payment, order, and inventory issues.
        </p>
      </header>
      <AdminCommandPalette />
        ...
        <div className="grid gap-2">
          {summary.issues.length === 0 ? (
            <div className="border border-emerald-300 bg-emerald-50 px-3 py-3 text-sm text-emerald-900">
              No diagnostic issues found in the checked window.
            </div>
          ) : (
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <form className="flex flex-wrap gap-2" action="/admin/dev-diagnostics">
            <input
              name="slug"
                ...
                ledgerEvents.map((event) => (
                  <tr key={event.id}>
                    ...
                    <td className="px-3 py-3">
                      <a className="font-semibold text-neutral-900 underline" href={`/admin/dev-diagnostics?slug=${encodeURIComponent(event.slug)}`}>
                        {event.slug}
                      </a>
                    </td>
                    <td className="px-3 py-3">
